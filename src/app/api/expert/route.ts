...
export async function POST(req: NextRequest) {
    try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/${apiRoutes.expert.add}`, {
            method: 'POST',
            headers: {
...
export async function GET(req: NextRequest) {
    try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/${apiRoutes.expert.get}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
